...
function printDates(data) {
    ...
    var options = d3.select("#Año").append("select").attr("multiple", "multiple").on('change',changeOption)
    options
        .selectAll("option")
        .data(data)
        .enter()
...
function printProducts(data) {
    var data = data._items
    options = d3.select("#Producto").append("select").on('change',changeOption)
    options
        .selectAll("option")
        .data(data)
        .enter()
...
function printRegions(data) {
    data = data._items
    options = d3.select("#Región").append("select").on('change',changeOption)
    options
        .selectAll("option")
        .data(data)
        .enter()
